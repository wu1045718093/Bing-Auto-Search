...
let mobileUserAgent = process.env.BingAutoSearch_mobileUserAgent || "Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.2 Mobile/15E148 Safari/604.1";
...
const getuserinfo = function (terminal) {
    return axios_bing.get('https://rewards.bing.com/api/getuserinfo/', {})
        .then(function (response) {
            if (response.status == 200 && response.data.dashboard.userStatus.counters.shopAndEarn[0].name.indexOf("ZHHK") != -1) {
                try {
            ...
            } else if (response.status == 200 && response.data.dashboard.userStatus.counters.shopAndEarn[0].name.indexOf("ZHHK") == -1) {
                console.log('当前非香港Rewards: ' + response.data.dashboard.userStatus.counters.shopAndEarn[0].name);
                process.exit();
            } else {
                console.log('未知错误');
                console.log(response.data);
...
const main = async(Terminal, terminal, UserAgent) => {
    await sleep(Math.random() * sleep_sec * 1000 + sleep_sec * 1000 / 2);
